...
- **Internal event handlers** (`onInvoicePaid`, `onInvoicePaymentFailed`): Accept `RondelDependencies` record, then domain event types (after transformation from Contract types); return `Result<unit, string>` for error propagation; marked `internal`, not exposed in `.fsi`
