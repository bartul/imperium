...
- `src/Imperium` is organized as one folder per bounded context, each owning its own namespace. Compile order (mirrors `Imperium.fsproj`):
  ...
  - `Gameplay/` — `Types.fsi/.fs`, `Commands.fsi/.fs`, `Events.fsi/.fs`, `State.fsi/.fs`, `Dependencies.fsi/.fs`, `Gameplay.fsi/.fs` (`namespace Imperium.Gameplay`, `[<RequireQualifiedAccess>] module Gameplay` facade)
...
- **Domain modules:** CQRS bounded contexts with `.fsi` files defining public APIs
  - Gameplay owns `GameId`, `NationId`, `PlayerId`, and `PlayerRoster` in `Gameplay/Types.fsi/.fs`; `GameStatus`, `GameInitialization`, and `GameplayState` live in `Gameplay/State.fsi/.fs`. Rondel exposes `Action`, `RondelBillingId`, and `Space`; `RondelBillingId.ofId` enables creating billing IDs from `Id` for in-process event conversion.
