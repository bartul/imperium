...
- Rondel domain context: rondel slots belong to nations (not players). `Space` cases cover the board order, with `ProductionOne/Two` and `ManeuverOne/Two` mapping to the same `Action`. Current events: `RondelCreated` and `NationActionDetermined` (carries `NationId * Action`). `createRondel` takes the set of participating `NationId`s; move/payment flows are stubbed for now.
