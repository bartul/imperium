...
- **Contract modules:** Cross-bounded-context communication types in `namespace Imperium.Contract`; no `.fsi` files (intentionally public); organized by bounded context under `src/Imperium/Contract/`
  - `Contract.Gameplay` (Contract/Gameplay.fs): StartGameCommand, GameplayCommand (routing DU), GameplayEvent (SetupCompleted)
...
- **Domain modules:** CQRS bounded contexts with `.fsi` files defining public APIs
  ...
  - `Imperium.Gameplay` exposes (at namespace level): transformation modules (StartGameCommand, GameplayEvent, RondelPositionedAtStartInboundEvent, SetRondelToStartingPositionsOutboundCommand), domain identity/value types (GameId, NationId, PlayerId, PlayerRoster), state types (GameStatus, GameInitialization, GameplayState), command routing DU (GameplayCommand), native integration event routing DU (GameplayEvent), inbound event routing DU (GameplayInboundEvent), outbound command routing DU (GameplayOutboundCommand), and dependency types (LoadGameplayState, GameplayEffects, CommitGameplayEffects, GameplayDependencies). The `[<RequireQualifiedAccess>] module Gameplay` facade exposes `val execute` and `val handle` as compiling stubs pending tests and implementation.
