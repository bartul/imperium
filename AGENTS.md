...
- `src/Imperium` is organized as one folder per bounded context, each owning its own namespace. Compile order (mirrors `Imperium.fsproj`):
  ...
  - `Accounting/` — `Types.fsi/.fs`, `Dependencies.fsi/.fs`, `Handlers.fs`, `Accounting.fsi/.fs` (`namespace Imperium.Accounting`, `[<RequireQualifiedAccess>] module Accounting` facade)
  - `Rondel/` — `Types.fsi/.fs`, `State.fsi/.fs`, `Dependencies.fsi/.fs`, `Movement.fs`, `Invoices.fs`, `Handlers.fs`, `Queries.fs`, `Rondel.fsi/.fs` (`namespace Imperium.Rondel`, `[<RequireQualifiedAccess>] module Rondel` facade)
...
- **FS0250 caveat.** Within a single assembly F# disallows a type and a same-named module (its companion) living in different files. Transformation modules (`SetToStartingPositionsCommand`, `MoveCommand`, `AccountingEvent`, etc.) therefore live in the *same* `.fsi`/`.fs` pair as the type they accompany — typically `Types.fsi/.fs` or `State.fsi/.fs` — rather than in a separate `Transformations.fsi/.fs`.
...
- Accounting internal structure: Stateless skeleton with no persistent state, split across `Accounting/Types.fsi/.fs` (types, dependencies, contract transformations) and `Accounting/Handlers.fs` (internal command handlers). `Handlers.chargeNationForRondelMovement` auto-approves charges (immediately publishes `RondelInvoicePaid` event), `Handlers.voidRondelCharge` is no-op. Future implementation can add balance tracking, transaction history, and payment validation.
