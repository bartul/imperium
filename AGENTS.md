...
- `src/Imperium` is organized as one folder per bounded context, each owning its own namespace. Compile order (mirrors `Imperium.fsproj`):
  ...
  - `Gameplay/` — `GameId.fsi/.fs`, `NationId.fsi/.fs`, `PlayerId.fsi/.fs`, `PlayerRoster.fsi/.fs`, `Commands.fsi/.fs`, `GameplayEvent.fsi/.fs`, `GameplayInboundEvents.fsi/.fs`, `GameplayState.fsi/.fs`, `Dependencies.fsi/.fs`, `Gameplay.fsi/.fs` (`namespace Imperium.Gameplay`, `[<RequireQualifiedAccess>] module Gameplay` facade)
