...
- `src/Imperium` contains the domain modules (`Primitives.fs`, `Gameplay.fs`, `Accounting.fs`, `Rondel.fs`); keep shared rules and calculations here.
...
- `Primitives.Amount` is a measured struct wrapper (`int<M>`) with guarded construction; errors are plain strings (e.g., negative amount); includes `tryParse` for string inputs; moved from Economy to Primitives as a reusable foundational type.
- Internal types like `RondelBillingId` (struct DU wrapping `Guid`) and `GameId`/`NationId` are implementation details hidden from public APIs; modules expose only commands, event handlers, and integration events.
- `Gameplay.NationId` (internal) is a DU for Imperial nations (Germany, Great Britain, France, Russia, Austria-Hungary, Italy) with `toString`/`tryParse` helpers; `all` is a `Set<NationId>`.
...
- Rondel domain context: CQRS bounded context where commands are identified by `GameId` (internal state hidden). Rondel slots belong to nations (not players). `Space` cases cover the board order, with `ProductionOne/Two` and `ManeuverOne/Two` mapping to the same `Action`. Integration events: `PositionedAtStart` (gameId), `ActionDetermined` (gameId, nation, action), `MovementToActionRejected` (gameId, nation, space). Commands: `setToStartPositions` initializes nations at start positions; `move` initiates movement and issues invoice to Accounting domain; event handlers `onInvoicedPaid`/`onInvoicePaymentFailed` process Accounting responses. All flows currently stubbed.
...
- Current test coverage: `RondelBillingId` (create, newId, toString, tryParse) - 9 tests passing.
