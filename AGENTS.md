...
- `src/Imperium` is organized as one folder per bounded context, each owning its own namespace. Compile order (mirrors `Imperium.fsproj`):
  ...
  - `Gameplay/` — `GameId.fsi/.fs`, `NationId.fsi/.fs`, `PlayerId.fsi/.fs`, `PlayerRoster.fsi/.fs`, `Commands.fsi/.fs`, `Events.fsi/.fs`, `State.fsi/.fs`, `Dependencies.fsi/.fs`, `Gameplay.fsi/.fs` (`namespace Imperium.Gameplay`, `[<RequireQualifiedAccess>] module Gameplay` facade)
...
- `tests/Imperium.UnitTests` contains Expecto-based unit tests; the project mirrors `src/` and isolates a package-ready spec support layer:
  ...
  - `Imperium/Gameplay/` — per-type unit tests (`GameIdTests.fs`, `NationIdTests.fs`, `PlayerIdTests.fs`, `PlayerRosterTests.fs`) plus CE-based behavior specs (`Context.fs`, `Assertions.fs`, `Specification.fs`, in namespace `Imperium.UnitTests.Gameplay`); per-type test files carry the `Tests` suffix while their module names match the bare type (e.g. `module Imperium.UnitTests.Gameplay.GameId`)
...
- **Domain modules:** CQRS bounded contexts with `.fsi` files defining public APIs
  - Gameplay owns `GameId` in `Gameplay/GameId.fsi/.fs`; `NationId` in `Gameplay/NationId.fsi/.fs`; `PlayerId` in `Gameplay/PlayerId.fsi/.fs`; `PlayerRoster` in `Gameplay/PlayerRoster.fsi/.fs`; `GameStatus`, `GameInitialization`, and `GameplayState` live in `Gameplay/State.fsi/.fs`. Rondel exposes `Action`, `RondelBillingId`, and `Space`; `RondelBillingId.ofId` enables creating billing IDs from `Id` for in-process event conversion.
  ...
  - `Imperium.Gameplay` exposes (at namespace level): transformation modules (StartGameCommand, GameplayEvent, RondelPositionedAtStartInboundEvent, SetRondelToStartingPositionsOutboundCommand), domain identity/value types (`GameId`, `NationId`, `PlayerId`, `PlayerRoster` as `Set<PlayerId>`), state types (GameStatus, GameInitialization, GameplayState), command routing DU (GameplayCommand), native integration event routing DU (GameplayEvent), inbound event routing DU (GameplayInboundEvent), outbound command routing DU (GameplayOutboundCommand), and dependency types (LoadGameplayState, GameplayEffects, CommitGameplayEffects, GameplayDependencies). The `GameplayEffects` record remains public as the commit-boundary payload, while its pipeline-style companion builder module (`none`, `withState`, `create`, `withEvent`, `withCommand`) is assembly-internal. Inbound transformations (`StartGameCommand.fromContract`, `RondelPositionedAtStartInboundEvent.fromContract`) are implemented using the FsToolkit `result {}` CE. `SetRondelToStartingPositionsOutboundCommand.toContract` and `GameplayEvent.toContract` are implemented. The `[<RequireQualifiedAccess>] module Gameplay` facade exposes `val execute` (StartGame branch: new-game emits the Rondel starting-positions outbound command sourced from `NationId.all`; existing-game is an idempotent no-op) and `val handle` (RondelPositionedAtStart branch: records Rondel setup completion and emits `SetupCompleted` once). Internal pure handlers (e.g. `module internal Handlers.startGame`) live inside `Gameplay.fs`, above the facade.
...
- Gameplay public API: Two routers (`Gameplay.execute` for commands, `Gameplay.handle` for inbound events) on the `[<RequireQualifiedAccess>] module Gameplay` facade. Interface/model files are split across `GameId`, `NationId`, `PlayerId`, `PlayerRoster`, `Commands`, `Events`, `State`, `Dependencies`, and facade files. Value types (`GameId`, `NationId`, `PlayerId`, `PlayerRoster` as `Set<PlayerId>`) and inbound transformations (`StartGameCommand.fromContract`, `RondelPositionedAtStartInboundEvent.fromContract`, both using the `result {}` CE) are implemented. `Gameplay.execute` handles the `StartGame` branch (emits `SetRondelToStartingPositions` with `NationId.all` for a new game; idempotent no-op for an already-started game), `Gameplay.handle` handles `RondelPositionedAtStart` (records Rondel setup completion and emits `SetupCompleted` once), and `SetRondelToStartingPositionsOutboundCommand.toContract` / `GameplayEvent.toContract` are implemented.
...
- Current test coverage snapshot:
  ...
  - **Imperium/Gameplay/Specification.fs** (4 CE-based spec expectations across 2 specs):
