...
- `src/Imperium` is organized as one folder per bounded context, each owning its own namespace. Compile order (mirrors `Imperium.fsproj`):
  ...
  - `Accounting/` — `Types.fsi/.fs`, `Handlers.fs`, `Accounting.fsi/.fs` (`namespace Imperium.Accounting`, `[<RequireQualifiedAccess>] module Accounting` facade)
  - `Rondel/` — `Types.fsi/.fs`, `State.fsi/.fs`, `Movement.fs`, `Invoices.fs`, `Handlers.fs`, `Queries.fs`, `Rondel.fsi/.fs` (`namespace Imperium.Rondel`, `[<RequireQualifiedAccess>] module Rondel` facade)
