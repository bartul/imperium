...
- **Pure business logic separated from IO** — handler internals return `RondelEffects` (state, integration events, outbound commands); the public router invokes a single injected `Commit` function — a `RondelDirectCommit` adapter in the terminal sandbox — that sequences all side effects at one infrastructure-owned seam
