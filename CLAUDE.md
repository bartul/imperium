...
- Terminal app (Phase 1, in progress): `Imperium.Terminal` project with `IBus` interface (generic `Publish<'T>`/`Subscribe<'T>` for events), thunk injection for cross-BC commands (`DispatchToAccounting`), `RondelHost`/`AccountingHost` skeletons, `InMemoryRondelStore`. Bus implementation complete (Dictionary-based, type-keyed handlers). `TerminalBusTests` (4 passing), `RondelHostTests` (5 pending). See `docs/rondel_multi_environment_architecture.md` and GitHub issue #47.
