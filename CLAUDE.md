...
- Rondel state: persistence uses `Contract.Rondel.RondelState`; domain uses `Rondel.RondelState` with `Id`, `Space option`, `RondelBillingId`, mapped via `RondelState.toContract/fromContract`.
