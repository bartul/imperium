...
- Terminal app (Phase 1, in progress): `Imperium.Terminal` project with `IBus` interface (generic `Publish<'T>`/`Subscribe<'T>` for events), thunk injection for cross-BC commands (`DispatchToAccounting`), `RondelHost`/`AccountingHost` skeletons, `InMemoryRondelStore`. Tests use `ptestCase` (pending). See `docs/rondel_multi_environment_architecture.md` and GitHub issue #47.
...
- Tests organized by concern: `RondelContractTests.fs` (5 transformation validation tests: SetToStartingPositionsCommand/MoveCommand `fromContract` input validation), `RondelTests.fs` (23 handler behavior tests using router pattern), `AccountingContractTests.fs` (6 transformation validation tests: ChargeNationForRondelMovementCommand/VoidRondelChargeCommand `fromContract` input validation), and `AccountingTests.fs` (2 handler behavior tests). Handler tests use record helpers (`Rondel`, `Accounting`) with routers (sync wrappers using `Async.RunSynchronously`), factory functions return router record + observable collections (events, commands) for verification. Tests call routers with union types (e.g., `rondel.Execute <| SetToStartingPositions cmd`, `accounting.Execute <| ChargeNationForRondelMovement cmd`). Property tests validate move behavior across random nations/spaces (15 iterations each). Total: 36 tests, all passing.
